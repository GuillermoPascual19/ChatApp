...
const Home = () => {
  ...
  const saveHistoryToLocalStorage = (chatHistory: string[], fileHistory: FileMessage[]) => {
  ...
  const addPeer = (incomingSignal: string, callerID: string) => {
    ...
    peer.on('data', (data) => {
      setChat(prev => {
        const newChat = [...prev, data.toString()];
        saveHistoryToLocalStorage(newChat, files);
        return newChat;
      });
    });
  ...
  const handleSendMessage = async () => {
    ...
    setChat(prev => {
      const newChat = [...prev, formattedMessage];
      saveHistoryToLocalStorage(newChat, files);
      return newChat;
    });
  ...
  useEffect(() => {
    ...
    socket.current.on('history', (history) => {
      setChat(history);
      saveHistoryToLocalStorage(history, files);
    });
    ...
    socket.current.on('new-file', (fileData: FileMessage) => {
      setFiles(prev => {
        ...
        saveHistoryToLocalStorage(chat, newFiles);
        return newFiles;
      });
    });
