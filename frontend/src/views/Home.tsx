...
const handleSendMessage = async () => {
  ...
  if (file) {
    try {
      ...
      socket.current.emit('message', {
        message: formattedMessage,
        channel: currentChannel,
        file: fileInfo // Enviamos toda la información del archivo
      });
    } catch (error) {
      console.error('Error processing file:', error);
      alert('Error processing file');
  ...
  socket.current.on('full-history', ({ messages, files }) => {
    console.log('Received full history from coordinator');
    setChat(messages);
    setFiles(files);
  });
  ...
  if (myID && username) {
    ...
    // Solicitar historial explícitamente después de unirse
    const timeout = setTimeout(() => {
      socket.current.emit('request-history', currentChannel);
    }, 1000);
    
    return () => clearTimeout(timeout);
  }
}, [currentChannel, myID, username]);
